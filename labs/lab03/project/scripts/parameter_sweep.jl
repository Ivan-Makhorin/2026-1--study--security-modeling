# # Параметрическое исследование: влияние плотности рёбер на граф атак
#
# **Цель**: изучить, как количество путей, максимальная in‑degree и средняя длина пути
# зависят от вероятности случайного ребра `edge_prob`.
...
for p in edge_probs
    ...
    push!(results, (p=p, paths=length(paths), max_indeg=max_indeg, avg_len=avg_path_len))
end
...
p1 = plot(
    edge_probs, [r.paths for r in results],
    marker = :circle,
...
# ## График средней длины пути
p2 = plot(
    edge_probs, [r.avg_len for r in results],
    marker = :circle,
    xlabel = "Плотность рёбер",
...
println("График сохранён в plots/parameter_sweep.png")