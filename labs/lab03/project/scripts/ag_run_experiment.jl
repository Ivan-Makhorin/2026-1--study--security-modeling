# # Запуск однократного эксперимента с графом атак
#
# **Цель**: построить граф, найти все пути, вычислить метрики и наиболее вероятный путь,
# сохранить результаты в JLD2.

# ## Инициализация проекта и загрузка модулей
using DrWatson
...
# ## Выполнение или загрузка ранее сохранённых данных
if isfile(filename)
    @load filename data
    println("Данные загружены из $filename")
else
    g = build_attack_graph(params[:n], params[:edge_prob], params[:cvss_scores], params[:trust_relations])
