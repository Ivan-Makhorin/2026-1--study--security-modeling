# # Анализ и визуализация результатов эксперимента
# 
# Загружает сохранённые данные и строит диагностические графики.
# 
...
# ## Параметры эксперимента (должны совпадать с run_experiment.jl)

params = Dict(:λ => 5.0, :T => 24.0, :num_hours_for_est => 10000)

filename = datadir("attack_sim", savename(params, "jld2"))
if !isfile(filename)
    error("Файл $filename не найден. Сначала запустите scripts/run_experiment.jl")
end
...
# ## График 3: Гистограмма интервалов между атаками
...
# ## График 4: QQ-plot интервалов против экспоненциального распределения
